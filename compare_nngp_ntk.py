...
parser.add_argument('--NTK_depth', type=int, default= 8,
...
spectrum_NNGP = compute_NTK_spectrum.get_effective_spectrum([depth], kmax, d, ker = 'NNGP')[0,:]

plt.rcParams.update({'font.size': 12})

print(spectrum[0:10])
print(spectrum_NNGP[0:10])
plt.loglog(np.linspace(1,len(spectrum),len(spectrum)), spectrum / spectrum[0], label = 'NTK')
plt.loglog(np.linspace(1,len(spectrum),len(spectrum)), spectrum_NNGP / spectrum_NNGP[0], label = 'NNGP')
...
sol_NNGP, p_NNGP = approx_learning_curves.simulate_uc(spectrum_NNGP, degens, lamb = lamb)
...
all_errs_NNGP = np.zeros((len(P_vals), kmax))
for i in range(len(P_vals)):
    P = P_vals[i]
    all_errs[i,:], all_mc[i], std_errs[i,:], std_MC[i] = generalization(P, P, spectrum, kmax, d, num_repeats, lamb=lamb)
    all_errs_NNGP[i,:], _ ,_ ,_ = generalization(P,P, spectrum_NNGP, kmax, d, num_repeats, lamb = lamb)
...
for i, k in enumerate([1,2,3]):
    if i == 0:
        plt.loglog(P_vals, all_errs[:,k] / all_errs[0,k], 'o', label = 'NTK', color = colors[i])
        plt.loglog(P_vals, all_errs_NNGP[:,k]/all_errs_NNGP[0,k], '^', label = 'NNGP', color = colors[i])
        plt.loglog(p, sol[:,k]/sol[0,k], '--', color = colors[i], label = 'k=%d' % k)
        plt.loglog(p, sol_NNGP[:,k] / sol_NNGP[0,k], '-.', color = colors[i])
    else:
        plt.loglog(P_vals, all_errs[:,k] / all_errs[0,k], 'o', color = colors[i])
        plt.loglog(p, sol[:,k]/sol[0,k], '--', color = colors[i], label = 'k=%d' % k)
        plt.loglog(P_vals, all_errs_NNGP[:,k]/all_errs_NNGP[0,k], '^', color = colors[i])
        plt.loglog(p, sol_NNGP[:,k] / sol_NNGP[0,k], '-.', color = colors[i])
plt.legend()
plt.xlabel(r'$P$',fontsize=20)
plt.ylabel(r'$E_k(P)$',fontsize = 20)
plt.tight_layout()
plt.savefig('results/ntk_vs_nngp_top3k.pdf')
plt.show()
