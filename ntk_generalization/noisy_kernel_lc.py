...
def generalization(P, X_teach, spectrum, kmax, d, num_repeats, lamb = 1e-6):
    ...
    for i in range(num_repeats):
        ...
        y = K_stu_te @ alpha_teach + sigma * np.random.normal(P)
...
parser.add_argument('--lamb', type=float,
  help='explicit regularization penalty', default = 1e-10)
parser.add_argument('--NTK_depth', type=int, default= 2,
...
sol, noise, total_noise, p = approx_learning_curves.simulate_uc_noise(spectrum, degens, lamb = lamb)

for i in range(4):
    plt.loglog(p, sigma**2 * noise[:,i], label = 'k=%d' % (i))
plt.legend()
plt.xlabel(r'$p$')
plt.ylabel(r'$E_{noise}$')
plt.savefig('results/noise_error_theory.pdf')
plt.show()
...
all_errsdf.to_csv('results/noisy_all_errs_lamb%lf_d%d.csv' % (lamb, d))
std_errsdf.to_csv('results/noisy_std_errs_lamb%lf_d%d.csv' % (lamb, d))
mc_df.to_csv('results/noisy_mc_errs_lamb%lf_d%d.csv' % (lamb, d))
std_mc_df.to_csv('results/noisy_std_mc_errs_lamb%lf_d%d.csv' % (lamb, d))
...
for i in range(len(kplot)):
    ...
    plt.plot(p, np.log10( (sol[:,kplot[i]] + sigma**2 * noise[:,kplot[i]]) * P_teach) , color = colors[i])
...
plt.tight_layout()
plt.savefig('results/noisy_kernel_expt_learning_curve_lamb%lf_d%d.pdf' % (lamb, d))
plt.show()


plt.errorbar(P_vals, np.log10(all_mc), std_MC/all_mc, fmt = 'o', linestyle = 'none', label = 'expt test')
plt.plot(p, np.log10(np.sum(sol, axis=1) * P_teach + P_teach * sigma**2 * total_noise), label= 'continuous approximation')
plt.xscale('log')
plt.xlim([np.amin(P_vals), np.amax(P_vals)])
plt.legend()
plt.xlabel(r'p')
plt.ylabel(r'$\log \ E_g$')
plt.savefig('results/noisy_sup_mode_vs_test_lamb%lf_d%d.pdf' % (lamb,d))
plt.show()
